...
const isEnabled = computed<boolean>({
  get: () => vhApp.data.userSettings.splitTunneling.useIpViaDevice,
  set: async (value: boolean) => {
    vhApp.data.userSettings.splitTunneling.useIpViaDevice = value;
  ...
  <v-sheet v-else>
    ...
    <config-card class="pb-2">
      <v-card-item>
        ...
        <p class="text-body-small text-disabled">{{ locale('SPLIT_IPS_VIA_DEVICE_SHORT_DESC') }}</p>
      </v-card-item>
    </config-card>
    <split-ip-input
