  ...
  >
    ...
    <v-row
      ...
      <v-col cols="12" :class="'text-center state-' + [vhApp.data.connectionState.toLowerCase()]">
        ...
        <!-- Connect button -->
        <v-btn id="connectBtn" height="40px" min-width="180px" rounded="pill"
          ...
          class="font-weight-bold mt-5 mb-4" :class="[vhApp.isConnectApp() ? 'connect-app' : 'client-app',
          { 'connected': vhApp.data.isConnected }, { 'tv-device': vhApp.data.features.isTv }]" :text="connectButtonText()"
      ...
      <v-col cols="12">
        ...
        <!-- Servers button -->
        <home-config-btn id="serverButton" prepend-icon="mdi-earth" tabindex="5" class="align-center mb-1" @click="!vhApp.data.features.isAddAccessKeySupported && vhApp.data.clientProfileInfos.length < 2
          && vhApp.data.clientProfileInfos[0].locationInfos.length < 2
          ? vhApp.showErrorMessage(locale('NO_ADDITIONAL_LOCATION_AVAILABLE'))
          : router.push({ name: 'SERVERS' })">
          <span class="config-btn-title" tabindex="-1">{{ vhApp.isSingleServerMode() ? locale('LOCATION') :
            locale('SERVER')
            }}</span>
        ...
        <home-config-btn id="excludeCountryButton" prepend-icon="mdi-call-split" class="mb-1" tabindex="6"
          ...
          <template v-if="!vhApp.data.userSettings.tunnelClientCountry && vhApp.data.state.clientCountryCode"
            v-slot:append>
            <span class="overflow-hidden d-inline-flex align-center justify-center ms-1"
              style="width: 23px; height: 15px; border-radius: 3px">
        ...
        <!-- App filter button -->
        <home-config-btn v-if="vhApp.data.features.isExcludeAppsSupported || vhApp.data.features.isIncludeAppsSupported"
          prepend-icon="mdi-call-split" class="mb-1" tabindex="7" @click="router.push({ name: 'APP_FILTER' })">
        ...
        <!-- Protocol button -->
        <home-config-btn id="protocolButton" prepend-icon="mdi-transit-connection-variant" tabindex="8"
          @click="router.push({ name: 'PROTOCOLS' })" class="align-center">
          ...
          <template v-if="vhApp.data.userSettings.useTcpProxy" v-slot:append>
            <v-chip :text="locale('CLOAK')" color="white" variant="tonal" size="small" density="compact"
              class="text-capitalize opacity-50 px-2" tabindex="-1" />
          </template>
    ...
    <v-dialog v-model="isShowDebugDialog" :persistent="true">
      <v-card color="background" title="Only developers" append-icon="mdi-bug-outline">
        ...
        <v-card-item>
          <!-- Debug data-1 -->
          <v-combobox theme="dark" v-model="debugData1" clearable label="DebugData1"
            :items="vhApp.data.features.debugCommands" hide-details hide-selected chips
            :list-props="{ bgColor: 'app-bar' }" closable-chips multiple class="mb-4" />

          <!-- Debug data-2 -->
          <v-combobox v-model="debugData2" clearable label="DebugData2" variant="filled" hide-details class="mb-4" />

          <!-- Open log file -->
          <btn-style-1 block text="Open log" :href="vhApp.data.serverUrl + UiConstants.logFileLocation"
            target="_blank" />
        </v-card-item>
